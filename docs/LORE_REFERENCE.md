...
| **Tábortűzi szájhagyomány (I–V.)** | `content/lore/campfire-stories.yml` + `CampfireStoryCatalog` + `CampfireStoryListener`: 9 hosszú, többütemű story; mindegyiknek `common/RED/BLUE/NEUTRAL/DARK` változata van. A frakciós verziók a kánontényt nem írják át, csak értelmezik és torzítják; egy konkrét campfire egyszerre egy story+perspective sessiont futtat minden hallgatónak. A nyílt pool nem fedheti fel a Suttogó-mechanikát, és az Első Csend valódi természetét / a harmadik mondat tartalmát továbbra sem magyarázhatja meg. A `available-during-prologue` story-flaget a `PrologueContentPolicy` érvényesíti; a `ket_mondat` Season 0 alatt tiltott. |
