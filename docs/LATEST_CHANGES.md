...
## 2026-09-20 — Mob attribution és ability hardening

- A pet/minion kill most központi, Folia-safe owner snapshotból oldódik fel minden reward/progression/tracking ágon; a saját pet/minion halála továbbra sem ad vanilla vagy custom jutalmat.
- A pet/minion boss-sebzése owner contributionként kerül a world-boss ledgerbe; az ability direct damage mob source-t használ.
- A legacy summon authored spawn lifecycle-ra, `RewardOwner.NONE`-ra és parent cleanupra került; az ally buff encounter/group szerint szűr.
- Javult a natural ability attach, a valódi nearest-player választás, a threshold retry, az `ON_DAMAGED` lefedettség, a direct ability level scaling, a summon presentation és a cross-region minion retarget retry.
- A mob level authority egységesen 200-as authored felső határt használ; az AOE line-of-sight viselkedése explicit config-kapcsolóval szabályozható.
...
- Az Enemy & World Boss Rework 2.0 a #140 combat-hardening branchre stackelve 93 stabil
  `MobTemplate` identityre és 61 bounded technique-re dolgozza át az authored PvE rostert.
  A natural világban 42 elérhető identity közül választ az EntityType mellett biome, dimenzió,
  mélység, napszak, időjárás, meglévő territory és eseménycontext alapján; Zombie carrierből
  hat, Skeletonből négy, Spiderből három eltérő viselkedésű változat létezik. A biome-native
  változatok csak a hozzájuk tartozó biome-családban kerülhetnek a választási poolba. A variant nem rank:
