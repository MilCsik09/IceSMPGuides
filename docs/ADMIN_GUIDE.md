...
| [ ] | CLIENT-14 Recept-böngésző parity | Fejlesztő | RECIPE_BROWSER kliens + `client.features.recipe-browser` kapu nyitva | BROWSE_RECIPES-re requestId-korrelált RECIPE_PAGE érkezik; a csempe-adatok (szintfeltétel, tervrajz-állapot, frakció-kötés, have/need, craftolhatóság) a vanilla /profession recipes csempéivel egyeznek ugyanabban a pillanatban; lap-index túlcsordulás clampelt, ismeretlen szakma REJECTED; craft üzenettípus nem létezik a protokollban; a lap-méret a `client.limits.recipe-page-size` kulcsból él | `client.features.recipe-browser: false` | `client/CLIENT-14/` |
