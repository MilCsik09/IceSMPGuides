...
| tábortűzi szájhagyomány | `content/lore/campfire-stories.yml` | restart |
...
`content/lore/campfire-stories.yml` fájlban él. Egy story stable ID-t, címet,
súlyt, kötelező `lore-anchor` mezőt, story-szintű `shared-beats` narratív
gerincet és öt `common/RED/BLUE/NEUTRAL/DARK` változatot kap.

A loader a `shared-beats` részt a kiválasztott variant első három authored
beatje után illeszti be. Így a tényleges történésnek van hosszú, közös gerince,
miközben a variant saját nyitása, frakciós értelmezése és lezárása megmarad.
Ne másold ötször ugyanazt a hosszú történeti részt a perspektívákba.

A bundled long-form szerződés:
- **Szájhagyomány:** legalább 100 szó és 7 beat perspektívánként.
- **Krónika:** legalább 150 szó és 9 beat perspektívánként.
- **Felső korlát:** legfeljebb 14 beat perspektívánként; ha ennél több kell,
  inkább szerkeszd feszesebbre a jeleneteket.
- A `shared-beats` semleges narratív gerinc; ne tegyél bele olyan
  frakcióspecifikus ítéletet, amely csak egy variantban lenne igaz.

A frakciós változat **nem alternatív kánon**: ugyanazt a bizonyított eseményt
más felelősség-, hős-, szégyen- és értelmezési hangsúllyal meséli. Ne írj át
évszámot, eseménykimenetet vagy szereplőazonosságot csak azért, mert másik
frakció beszél. Nyílt campfire contentben ne leplezd le a Suttogó-mechanikát,
az Első Csend valódi természetét vagy a harmadik mondat tartalmát.
...
campfire-stories:
  pelda:
    ...
    lore-anchor: III
    shared-beats:
      - {delay-ticks: 85, text: "<gray>„A közös történeti gerinc első jelenete.”</gray>"}
      - {delay-ticks: 95, text: "<gray>„A közös történeti gerinc folytatása.”</gray>"}
    variants:
      common:
        beats:
          - {delay-ticks: 0, text: "<dark_gray>A tűz halkabb lesz.</dark_gray>"}
          - {delay-ticks: 60, text: "<gray>„A közös tény első mondata.”</gray>"}
          - {delay-ticks: 70, text: "<gray>„A közös tény második mondata.”</gray>"}
          - {delay-ticks: 80, text: "<gray>„A common értelmezés.”</gray>"}
          - {delay-ticks: 60, text: "<dark_gray>A történet elcsendesedik.</dark_gray>"}
      red:
        beats:
          - {delay-ticks: 0, text: "<dark_gray>A láng magasabbra csap.</dark_gray>"}
          - {delay-ticks: 60, text: "<gray>„Ugyanaz az első kánoni tény.”</gray>"}
          - {delay-ticks: 70, text: "<gray>„Ugyanaz a második kánoni tény.”</gray>"}
          - {delay-ticks: 80, text: "<gray>„Pyralingradban ezt más hangsúllyal mesélik.”</gray>"}
          - {delay-ticks: 60, text: "<dark_gray>A vörös változat itt zárul.</dark_gray>"}
```

A campfire oral history **phase-independent**: nincs
`available-during-prologue` mező, és a selector nem használ
`PrologueContentPolicy`-t. Season 0 alatt ugyanaz a teljes nyílt katalógus
hallható. A valódi későbbi titkokat nem phase-flag mögé tedd, hanem eleve ne
authorold a publikus campfire poolba.

A `config/general.yml -> campfire-story.*` csak runtime tuningot tart:
gyülekezési idő, hallgatói sugár, join-in-progress, fire-session cooldown,
jutalom, presentation, pacing és faction-perspective esély. A történetszöveget
oda ne másold vissza.
